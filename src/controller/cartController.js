...
//===============router handler for create cart==============
const createCart = async function (req, res) {
    try {
        let userId = req.params.userId;

        if (!userId) {
            return res.status(400).send({ status: false, message: "Please give UserId in Url" });
        }
        ...
        let updateCart = await cartModel.findOneAndUpdate(
            { userId: userId },
            { items: x, totalPrice: totalPrices, totalItems: x.length },
            { new: true }
        );
...
//===============router handler for update cart==============
let updatecart = async function (req, res) {
    try {
        ...
        if (Object.keys(data).length == 0) {
            return res.status(400).send({ status: false, message: "No keys found for updation , please enter keys." });
        }
        ...
        if (productId) {
            if (!isValidString(productId)) {
                return res.status(400).send({ status: false, message: "product Id cannot be empty" });
            }
            if (!isValidObjectId(productId)) {
                return res.status(400).send({ status: false, message: "enter a valid product Id" });
            }
        }
        ...
        if (!cartdetails) {
            return res.status(404).send({ status: false, message: "cart doesnot exists create a new cart" });
        }
        ...
        const removeditems = await cartModel.findOneAndUpdate(
            { _id: cartId },
            { items: Arrayitems, totalPrice: totalprice, totalItems: totalItems },
            { new: true }
        ).select({ __v: 0 });
        if (!removeditems) {
...
//===============router handler for delete cart==============
